...
volumeGet() {
	...
	if [[ $( cat $dirshm/player ) == mpd && $mixertype == software ]]; then
		...
		if [[ ! $controls ]]; then
			...
			voldb=$( amixer -c 0 -M sget "$control" \
				| grep -m1 '%.*dB' \
				| sed 's/.*\[\(.*\)%\] \[\(.*\)dB.*/\1 \2/' )
			if [[ $voldb ]]; then
				volume=${voldb/ *}
...
volumeSetAt() {
	...
	if [[ $btclient ]]; then
		...
		amixer -c 0 -Mq sset "$control" $val%
	else
		mpc -q volume $val
	fi
}
...
playerstop )
	...
	if [[ -e $file ]]; then
		...
		[[ $db == 0.00 ]] && amixer -c 0 -Mq sset "$control" 0dB
		rm -f $file
	fi
...
volume0db )
	...
	volumeGet
	amixer -c 0 -Mq sset "$control" 0dB
	;;
volumecontrols )
...
volumeupdown )
	...
	if [[ -e $dirshm/btclient ]]; then
		...
		amixer -c 0 -Mq sset "$control" 1%$updn
	else
		mpc -q volume ${updn}1
	fi
	volumeGet
