...
lastcard=${proccardn: -1}
[[ $( ls -d /proc/asound/card[0-9]/usbmixer 2> /dev/null ) ]] && (( lastcard-- ))
while read path; do
	...
	CARD=${path: -1}
	[[ $CARD == $lastcard && -e $dirsystem/audio-output ]] && NAME=$( < $dirsystem/audio-output ) || NAME=$name
