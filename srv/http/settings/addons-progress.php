...
$addon    = $addons[ $alias ];
if ( $alias === 'cove' ) {
	$icon = '<i class="page-icon fa-coverart"></i>';
	$href = '/';
	$name = 'Album Thumbnails';
	$path = $type === '/' ? '' : $type; // path = $opt[ 1 ]
	$type = 'Update';
} else {
	$icon = '<i class="page-icon fa-jigsaw"></i>';
...
if ( $postinfo ) {
	...
	if ( $postinfo ) $postmsg  .= '<br><br><i class="fa-info-circle wh"></i>'.$postinfo;
}
...
<div id="infoOverlay" class="info hide">
	<div id="infoBox">
		<div id="infoTopBg"><div id="infoTop"><i class="fa-jigsaw"></i><a id="infoTitle"><?=$name?></a></div></div>
		<div id="infoContent"><div class="infomessage"><?=$postmsg?></div></div>
		<div class="infobtn infobtn-primary">OK</div>
	</div>
</div>
<br>
<p class="addontitle gr"><i class="titleicon fa-gear<?=( $localhost ? '' : ' blink' )?>"></i>&ensp;<wh><?=$name?></wh> - <?=$type?> ...</p>
<pre class="progress">
