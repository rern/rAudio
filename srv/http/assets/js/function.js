...
` );
		V.colorpicker = new KellyColorPicker( {
			...
			, userEvents : {
				change : e => {
					var hsl   = e.getCurColorHsl();
					var h     = hsl.h;
					var s     = hsl.s;
					var l     = hsl.l;
					var $root = $( ':root' );
					$.each( V.css.cm, ( k, v ) => $root.css( '--'+ k, 'hsl( '+ h +', '+ s +'%,'+ l +'% )' ) );
	...
	, option     : {
		...
		, list          : {
			...
			, playback      : {
				...
				, cover            : 'Cover art'
					, covervu      : V.vumeter +'As default'
				, volume           : 'Volume'
					, vumeter      : V.vumeter +'VU meter'
				, buttons          : 'Buttons'
		...
		INFO( {
			...
			, ok         : () => {
				...
				BASH( [ 'webradioadd', $( '#lib-path' ).text(), name, url, charset, 'CMD DIR NAME URL CHARSET' ], error => {
					BANNER_HIDE();
					if ( error ) WEBRADIO.exists( error, name, url, charset );
				} );
			}
		} );
	}
