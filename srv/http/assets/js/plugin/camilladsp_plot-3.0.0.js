...
- eval_filterconfig.py : https://github.com/HEnquist/pycamilladsp-plot/blob/master/camilladsp_plot/eval_filterconfig.py
- filters.py           : https://github.com/HEnquist/pycamilladsp-plot/blob/master/camilladsp_plot/filters.py

(require Math.js)
*/

// eval_filterconfig.py >> js ////////////////////////////////////////////////////////////////////
function evalFilter(filterconf, name = null, samplerate = 44100, npoints = 1000, volume = 0.0) {
    var fvect = logspace(1.0, samplerate * 0.95 / 2.0, npoints);
    if (name === null) {
        name = `unnamed ${filterconf.type}`;
    }
    var result = { name, samplerate, f: fvect };

    let currfilt;
    if (["Biquad", "DiffEq", "BiquadCombo"].includes(filterconf.type)) {
        if (filterconf.type === "DiffEq") {
            currfilt = new DiffEq(filterconf.parameters, samplerate);
        } else if (filterconf.type === "BiquadCombo") {
            currfilt = new BiquadCombo(filterconf.parameters, samplerate);
        } else {
            currfilt = new Biquad(filterconf.parameters, samplerate);
        }
        var [_, magn, phase] = currfilt.gainAndPhase(fvect);
        result.magnitude = magn;
        result.phase = phase;

    } else if (filterconf.type === "Conv") {
        currfilt = new Conv(filterconf.parameters || null, samplerate);
        var [_, magn, phase] = currfilt.gainAndPhase(fvect, true);
        var [t, impulse] = currfilt.getImpulse();
        result.magnitude = magn;
        result.phase = phase;
        result.time = t;
        result.impulse = impulse;

    } else if (filterconf.type === "Delay") {
        currfilt = new Delay(filterconf.parameters, samplerate);
        var [_, magn, phase] = currfilt.gainAndPhase(fvect);
        result.magnitude = magn;
        result.phase = phase;

    } else if (filterconf.type === "Gain") {
        currfilt = new Gain(filterconf.parameters);
        var [_, magn, phase] = currfilt.gainAndPhase(fvect);
        result.magnitude = magn;
        result.phase = phase;

    } else if (filterconf.type === "Loudness") {
        currfilt = new Loudness(filterconf.parameters, samplerate, volume);
        var [_, magn, phase] = currfilt.gainAndPhase(fvect);
        result.magnitude = magn;
        result.phase = phase;

    } else if (["Volume", "Dither"].includes(filterconf.type)) {
        ...
        throw new Error(`Unknown filter type ${filterconf.type}`);
    }

    var [f_grp, groupdelay] = calcGroupDelay(result.f, result.phase);
    result.f_groupdelay = f_grp;
    result.groupdelay = groupdelay;
    // result.phase = unwrapPhase(result.phase);
    
    return result;
}
function evalFilterStep(conf, pipelineindex, name = "filterstep", npoints = 1000, overrides = null) {
    ...
    var pipelinestep = conf.pipeline[pipelineindex];
    let totcgain = new Array(npoints).fill(1.0);
    pipelinestep.names.forEach(filt => {
        var filterconf = conf.filters[filt];
        let currfilt;
        ...
        totcgain = totcgain.map((cg, i) => cg * cgainstep[i]);
    });
    var gain = totcgain.map(cg => 20.0 * Math.log10(Math.abs(cg) + 1.0e-15));
    var phase = totcgain.map(cg => 180 / Math.PI * math.atan2(cg.im, cg.re));
    var [f_grp, groupdelay] = calcGroupDelay(fvect, phase);
    return {
        name,
        samplerate,
        f: fvect,
...
class BiquadCombo extends BaseFilter {
	...
	complexGain(freq, removeDelay = false) {
		let A = new Array(freq.length).fill(1.0);
...
class Conv {
	...
	findPeak() {
		let maxVal = Math.abs(this.impulse[0]);
		let idx = 0;
		for (let i = 1; i < this.impulse.length; i++) {
			let absVal = Math.abs(this.impulse[i]);
			if (absVal > maxVal) {
				maxVal = absVal;
				idx = i;
			}
	...
	complexGain( f, removeDelay = false ) {
		let impulselen = this.impulse.length;
		let npoints    = Math.pow( 2, Math.ceil( Math.log2( impulselen ) ) );
		if ( npoints < 1024 ) npoints = 1024;
		let impulse    = this.impulse.slice();
		let padding    = new Array( npoints - impulselen ).fill( 0.0 );
		impulse        = impulse.concat( padding );
		let impfft     = fft( impulse ); // Implement FFT function here
		let f_fft      = Array.from( { length: npoints / 2 }, ( _, n ) => this.fs * n / npoints );
		let cut        = impfft.slice( 0, npoints / 2 );
		if ( removeDelay ) {
			let maxidx = this.findPeak();
			cut = cut.map( ( val, idx ) => {
				math.complex( 0, 1 )
						.neg()
		...
		if ( f !== null ) {
			let interpolated = this.interpolatePolar( cut, f_fft, f );
	...
	interpolate(y, xold, xnew) {
		let ynew = [];
		for (let x of xnew) {
			let idx = (y.length * x) / xold[xold.length - 1];
			...
			let fract = idx - i1;
			let newval = (1 - fract) * y[i1] + fract * y[i2];
			ynew.push(newval);
		}
		return ynew;
	}
	...
	interpolatePolar(y, xold, xnew) {
		let y_magn = y.map(yval => math.abs(yval));
		let y_ang = y.map(yval => math.atan2(yval.im, yval.re) * 180.0 / Math.PI);
		y_ang = this.unwrapPhase(y_ang, 270.0).map(yval => math.pi * yval / 180.0);
		let y_magn_interp = this.interpolate(y_magn, xold, xnew);
		let y_ang_interp = this.interpolate(y_ang, xold, xnew);
		return y_magn_interp.map((r, i) => math.complex({ r, phi: y_ang_interp[i] }));
	}

	gainAndPhase(f, removeDelay = false) {
		let [f_fft, Avec] = this.complexGain(null, removeDelay);
		let interpolated = this.interpolatePolar(Avec, f_fft, f);
		let gain = interpolated.map(A => 20.0 * Math.log10(Math.abs(A) + 1.0e-15));
		let phase = interpolated.map(A => math.atan2(A.im, A.re) * 180.0 / Math.PI);
		return [f, gain, phase];
	}

	getImpulse() {
		let t = Array.from({length: this.impulse.length}, (_, n) => n / this.fs);
...
class DiffEq extends BaseFilter {
	...
	complex_gain( freq, remove_delay = false ) {
		var zvec = freq.map( f => {
			math.complex(0, 1).mul( 2 * Math.PI * f / this.fs).exp();
		} );
		var A1   = new Array( freq.length ).fill( 0.0 );
		this.b.forEach( ( bn, n ) => {
			A1 = A1.map( ( a1, i ) => zvec[ i ].pow( -n ).mul( bn ).add( a1 ) );
		} );
		var A2   = new Array( freq.length ).fill( 0.0 );
		this.a.forEach( ( an, n ) => {
			A2 = A2.map( ( a2, i ) => zvec[ i ].pow( -n ).mul( an ).add( a2 ) );
		} );
...
class Loudness extends BaseFilter {
	...
	complexGain(freq, removeDelay = false) {
		let A = new Array(freq.length).fill(this.midGain);
		this.biquads.forEach(bq => {
			var [_, Atemp] = bq.complexGain(freq);
			A = A.map((a, i) => a * Atemp[i]);
		});
