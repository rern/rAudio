...
function evalFilter( name, npoints = 1000, volume = 0.0 ) {
	var samplerate = DEV.samplerate;
	var filter     = FIL[ name ];
	var f          = logSpace( 1.0, samplerate * 0.95 / 2.0, npoints );
	var currfilt   = currentFilter( filter, samplerate );
...
function evalFilterStep( index, npoints = 1000, overrides = null ) {
	var samplerate = DEV.samplerate;
	if ( overrides ) {
		if ( overrides.samplerate !== undefined && DEV.resampler === undefined ) samplerate = overrides.samplerate;
	}
	var f          = logSpace( 10.0, samplerate * 0.95 / 2.0, npoints );
	var totcgain   = new Array( npoints ).fill( 1.0 );
	var currfilt;
	PIP[ index ].names.forEach( name => {
		currfilt         = currentFilter( FIL[ name ], samplerate );
...
function currentFilter( filter, samplerate ) {
	switch( filter.type ) {
		case 'Biquad':
