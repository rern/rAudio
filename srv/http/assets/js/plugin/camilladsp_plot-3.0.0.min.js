...
function evalFilter( name, samplerate = 44100, npoints = 1000, volume = 0.0 ) {
	...
	var currfilt   = currentFilter( filterconf, samplerate );
	if ( ! currfilt ) return -1
	
	var [ ma, ph ] = currfilt.gainAndPhase( fvect );
	var [ fg, gr ] = calcGroupDelay( fvect, ph );
	var result     = {
		  f            : fvect
		, f_groupdelay : fg
		, groupdelay   : gr
...
function evalFilterStep( index, npoints = 1000, overrides = null ) {
	...
	if ( overrides ) {
		if ( overrides.samplerate !== undefined && conf.devices.resampler === undefined ) samplerate = overrides.samplerate;
	}
	var fvect      = logSpace( 10.0, samplerate * 0.95 / 2.0, npoints );
	var totcgain   = new Array( npoints ).fill( 1.0 );
	var currfilt;
	conf.pipeline[ index ].names.forEach( name => {
		var filterconf   = S.config.filters[ name ];
		currfilt         = currentFilter( filterconf, samplerate );
		if ( ! currfilt ) return false
		
		var [ _, cgain ] = currfilt.complexGain( fvect );
...
function currentFilter( filterconf, samplerate ) {
	switch( filterconf.type ) {
		case 'Biquad':
			return new Biquad( filterconf.parameters, samplerate )
		case 'BiquadCombo':
			return new BiquadCombo( filterconf.parameters, samplerate )
		case 'Conv':
			return new Conv( filterconf.parameters || null, samplerate )
		case 'Delay':
			return new Delay( filterconf.parameters, samplerate )
		case 'DiffEq':
			return new DiffEq( filterconf.parameters, samplerate )
		case 'Gain':
			return new Gain( filterconf.parameters )
		case 'Loudness':
			return new Loudness( filterconf.parameters, samplerate, volume )
		case 'Dither':
		case 'Volume':
