function onPageInactive() {
	if ( $( '.divscan.hide' ).length === 2 ) return
	
	clearTimeout( V.timeoutscan );
	$( '.back' ).trigger( 'click' );
}
function renderPage() {
	SCAN.hide();
	[ 'bluetooth', 'wlan', 'lan' ].forEach( k => {
		if ( S.device[ k ] ) {
			RENDER[ k ]();
			$( '#div'+ k ).removeClass( 'hide' );
		} else {
			$( '#div'+ k ).addClass( 'hide' );
		}
	} );
	$( '#divinterface' ).removeClass( 'hide' );
	if ( ! S.ip ) {
		$( '#divwebui' ).addClass( 'hide' );
		return
	}
	
	if ( S.ap ) {
		$( '#qrap' )
			.html(
				 '<wh>'+ S.apconf.ssid
				+'<br>'+ S.apconf.passphrase +'</wh>'
				+ QRCode( S.apconf.qr )
			)
			.removeClass( 'hide' );
	} else {
		$( '#divap' ).addClass( 'hide', ! S.ap );
	}
	$( '#qrurl' ).html(
		  'http://<wh>'+ S.ip +'</wh>'
		 + ( S.hostname ? '<br>http://'+ S.hostname : '' )
		 + QRCode( 'http://'+ S.ip )
	);
	$( '#divwebui' ).removeClass( 'hide' );
	CONTENT();
}
var BT     = ( action, $li ) => {
	var icon  = $li.find( 'i' ).hasClass( 'i-btsender' ) ? 'btsender' : 'bluetooth';
	NOTIFY( icon, $li.data( 'name' ), COMMON.capitalize( action ) +' ...', -1 );
	BASH( [ 'settings/networks-bluetooth.sh', 'cmd', action, $li.data( 'id' ), 'CMD ACTION MAC' ] );
}
var CHANGE = {
	  connect : ip => {
		var href = 'http://IP/settings.php?p=networks';
		try {
			location.href = href.replace( 'IP', ip );
		} catch( error ) {
			setTimeout( () => CHANGE.connect( ip ), 1000 );
		}
	}
	, ip      : ( result, icon, title, val, callback ) => {
		var ip = val.ADDRESS;
		if ( result == -1 ) {
			clearTimeout( V.timeoutchangeip );
			BANNER_HIDE();
			INFO( {
				  icon    : icon
				, title   : title
				, message : 'IP <c>'+ ip +'</c> already in use.'
				, ok      : () => callback( val )
			} );
		}
	}
	, ssid    : ssid => {
		NOTIFY( 'wifi', ssid, 'Connect ...' );
		BASH( [ 'profileconnect', ssid, 'CMD ESSID' ] );
		$( 'li.active i' ).addClass( 'blink' );
	}
	, switch  : ip => {
		if ( ! $( 'li.active' ).hasClass( 'current' ) ) return
		
		var delay = 3000;
		if ( ! ip ) {
			ip    = S.hostname;
			delay = 10000;
		}
		NOTIFY( V.wlan ? 'wlan' : 'lan', 'Reconnect', 'rAudio @ http://'+ ip +' ...' );
		V.timeoutchangeip = setTimeout( () => CHANGE.connect( ip ), delay );
	}
}
var EDIT   = {
	  lan  : values => {
		if ( values ) {
			var dhcp = values.DHCP;
		} else {
			values   = COMMON.json.clone( S.list.lan );
			var dhcp = values.DHCP
			delete values.DHCP;
		}
		var $li   = $( 'li.active' );
		var icon  = 'lan';
		var title = 'LAN Connection';
		INFO( {
			  icon         : icon
			, title        : title
			, list         : [
				  [ 'IP',      'text' ]
				, [ 'Gateway', 'text' ]
			]
			, footer       : S.ip ===  location.hostname ? WLAN.warning( 'This is' ) : ''
			, values       : values
			, focus        : 0
			, checkchanged : true
			, checkip      : [ 0, 1 ]
			, beforeshow   : () => $( '.extrabtn' ).toggleClass( 'disabled', dhcp )
			, buttonlabel  : 'DHCP'
			, button       : () => {
				$li.find( 'i' ).addClass( 'blink' );
				NOTIFY( icon, title, 'Change ...' );
				BASH( [ 'lanedit' ] );
				CHANGE.switch();
			}
			, oklabel      : dhcp ? 'Static' : ''
			, ok           : () => {
				var val  = _INFO.val();
				$li.find( 'i' ).addClass( 'blink' );
				NOTIFY( icon, title, 'Change ...' );
				BASH( COMMON.cmd_json2args( 'lanedit', val ), result => {
					CHANGE.ip( result, icon, title, val, EDIT.lan );
				} );
				CHANGE.switch( val.ADDRESS );
			}
		} );
	}
	, wlan : values => {
		if ( ! values ) values = { ESSID: '', KEY: '', SECURITY: false, HIDDEN: false }
		var dhcp = ! ( 'ADDRESS' in values );
		var list = [
			  [ 'SSID',         'text' ]
			, [ 'Password',     'password' ]
			, [ 'IP',           'text' ]     // static - ADDRESS
			, [ 'Gateway',      'text' ]     // static - GATEWAY
			, [ 'WEP Protocol', 'checkbox' ]
			, [ 'Hidden SSID',  'checkbox' ]
		];
		var json = {
			  icon     : 'wifi'
			, title    : 'Wi-Fi Connection'
			, tablabel : [ 'DHCP', 'Static IP' ]
		}
		if ( dhcp ) {
			json.tab = [ '', () => {
				var v     = _INFO.val();
				var keys  = Object.keys( v );
				keys.splice( 2, 0, 'ADDRESS', 'GATEWAY' ); // insert in order
				v.ADDRESS = COMMON.ipSub( S.ip );
				v.GATEWAY = S.gateway;
				var val   = {};
				keys.forEach( k => val[ k ] = v[ k ] );
				EDIT.wlan( val );
			} ];
			list.splice( 2, 2 );
		} else {
			json.tab = [ () => {
				V.edit  = false;
				var val = _INFO.val();
				delete val.ADDRESS;
				delete val.GATEWAY;
				EDIT.wlan( val );
			}, '' ]
			json.checkip = [ 2, 3 ];
		}
		if ( V.edit ) {
			if ( ! dhcp ) json.focus = 2
			json.footer = WLAN.warning( 'This is' );
			json.checkchanged = true;
		} else {
			json.focus = 0
			json.checkchanged = false;
		}
		var editonly = V.edit === 'editonly';
		delete V.edit;
		INFO( {
				...json
			, boxwidth     : 180
			, list         : list
			, values       : values
			, checkblank   : [ 0 ]
			, checklength  : { 1: [ 8, 'min' ] }
			, ok           : () => {
				var val = _INFO.val();
				if ( editonly ) {
					val.EDITONLY = true;
				} else {
					CHANGE.switch( val.ADDRESS );
				}
				WLAN.connect( val );
			}
		} );
	}
}
var RENDER = {
	  bluetooth : () => {
		if ( LIST.equal( 'bluetooth' ) ) return
		
		var html  = '';
		var cls;
		if ( S.list.bluetooth ) {
			S.list.bluetooth.forEach( list => {
				cls   = list.connected ? ' current' : '';
				html += '<li class="bt'+ cls +'" data-id="'+ list.mac +'" data-name="'+ list.name +'">'
					  + ICON( list.type === 'Source' ? 'btsender' : 'bluetooth' ) +'<dot></dot>'+ list.name +'</li>';
			} );
		}
		LIST.render( 'bluetooth', html );
		$( '#divbluetooth' ).removeClass( 'hide' );
	}
	, lan       : () => {
		var html = '';
		if ( S.list.lan ) {
			var ipeth = S.list.lan.ADDRESS;
			var cls   = ipeth ? ' class="current"' : '';
			html      =  '<li data-ip="'+ ipeth +'"'+ cls +'>'
						+ ICON( 'lan' ) +'<dot></dot>'+ ipeth +'<gr>'+ RENDER.raquo + S.list.lan.GATEWAY +'</gr></li>';
		}
		$( '#lan' ).html( html );
		$( '#divlan' ).removeClass( 'hide' );
	}
	, raquo     : '&ensp;&raquo;&ensp;'
	, wlan      : () => {
		if ( LIST.equal( 'wlan' ) ) return
		
		var html = '';
		if ( S.ap ) {
			html +=  '<li class="wl ap" data-id="ap">'
					+ ICON( 'ap' ) +'<gr>Access point&ensp;&laquo;&ensp;</gr>'+ S.apconf.ip +'</li>';
		}
		if ( S.list.wlan ) {
			S.list.wlan.forEach( ( list, i ) => {
				if ( S.ap ) i++;
				var ssid  = list.ssid;
				html += '<li data-id="'+ ssid +'" class="wl';
				if ( list.ip ) {
					html +=  ' current" data-ip="'+ list.ip +'">'
							+ ICON( list.icon ) +'<dot></dot>'+ ssid +'<gr>'+ RENDER.raquo + list.ip + RENDER.raquo + list.gateway +'</gr></li>';
				} else {
					html +=  '">'
							+ ICON( 'wifi' ) +'<dot></dot>'+ ssid +'</li>';
				}
			} );
		}
		LIST.render( 'wlan', html );
		$( '#divwlan' ).removeClass( 'hide' );
		BANNER_HIDE();
	}
}
var SCAN   = {
	  bluetooth : () => {
		BASH( 'networks-scan.sh', data => {
			var htmlbt      = '';
			if ( data ) {
				var cls, icon;
				data.forEach( list => {
					icon   = ICON( 'bluetooth' );
					cls    = 'btscan';
					if ( list.current ) {
						cls  += ' current';
					} else if ( list.paired ) {
						cls  += ' profile';
					}
					htmlbt += '<li class="'+ cls +'" data-id="'+ list.mac +'" data-name="'+ list.name +'">'
							+ icon +'<dot></dot>'+ list.name +'</li>';
				} );
			} else {
				htmlbt       = '<li><gr>(no Bluetooth devices found)</gr></li>';
			}
			$( '#scanbluetooth' ).html( htmlbt );
			V.timeoutscan = setTimeout( SCAN.bluetooth, 12000 );
		}, 'json' );
	}
	, hide      : () => {
		$( '.divscan' )
			.addClass( 'hide' )
			.find( 'ul' ).empty();
		$( '.helphead, #divinterface, #divwebui' ).removeClass( 'hide' );
	}
	, wlan      : () => {
		BASH( 'networks-scan.sh wlan', data => {
			var htmlwl    = '';
			if ( data ) {
				var scan   = data.scan;
				scan.sort( ( a, b ) => b.signal - a.signal );
				var cls, signal;
				scan.forEach( list => {
					ssid    = list.ssid;
					signal  = list.signal;
					nwifi   = signal > -60 ? '' : ( signal < -67 ? 1 : 2 );
					cls     = 'wlscan';
					if ( ssid === data.current ) {
						cls  += ' current';
					} else if ( data.profiles && data.profiles.includes( ssid ) ) {
						cls  += ' profile';
					}
					if ( signal && signal < -67 ) ssid = '<gr>'+ ssid +'</gr>';
					htmlwl += '<li class="'+ cls +'" data-id="'+ ssid +'" data-encrypt="'+ list.encrypt +'" data-wpa="'+ list.wpa +'">'
							+ ICON( 'wifi'+ nwifi ) +'<dot></dot>'+ ssid;
					htmlwl += list.encrypt === 'on' ? ICON( 'lock' ) : '&ensp;';
					htmlwl += signal != 0 ? '<gr>'+ signal +' dBm</gr>' : '';
					htmlwl += '</li>';
				} );
			} else {
				htmlwl       = '<li><gr>(no access points found)</gr></li>';
			}
			$( '#scanwlan' ).html( htmlwl );
			V.timeoutscan = setTimeout( SCAN.wlan, 12000 );
		}, 'json' );
	}
}
var WLAN   = {
	  ap2ssid   : ( ssid, val ) => {
		var changeip = S.ap && S.apconf.ip === location.hostname;
		INFO( {
			  icon    : 'ap'
			, title   : 'Access Point'
			, message : '<p>· Disable and connect <wh>'+ ssid +'</wh>'
					   +'<br>· Connected clients will be dropped.'
					   + ( changeip ? '<br>· New rAudio URL: <c>http://'+ S.hostname +'</c>' : '' ) +'</p>'
			, ok      : () => {
				typeof val === 'object' ? WLAN.connect( val ) : CHANGE.ssid( ssid );
			}
		} );
	}
	, connect    : val => {
		var keys      = Object.keys( val );
		var values    = Object.values( val );
		$( 'li.active i' ).addClass( 'blink' );
		NOTIFY( I.icon, val.ESSID, V.edit ? 'Change ...' : 'Connect ...' );
		BASH( [ 'connect', ...values, 'CMD '+ keys.join( ' ' ) ], result => {
			CHANGE.ip( result, I.icon, I.title, val, EDIT.wlan );
		} );
	}
	, disconnect : () => {
		var ssid = $( 'li.active' ).data( 'id' );
		NOTIFY( 'wifi', ssid, 'Disconnect ...' );
		BASH( [ 'disconnect', ssid, 'CMD SSID' ] );
		$( 'li.active i' ).addClass( 'blink' );
	}
	, warning    : action => {
		var $li = $( 'li.active' );
		if ( $li.data( 'ip' ) === location.hostname ) return V.i_warning +'<wh>'+ action +' current connection</wh>'
	}
}

$( function() { // document ready start >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

$( '.back' ).on( 'click', function() {
	clearTimeout( V.timeoutscan );
	SCAN.hide();
	$( '.divscan .blink' ).removeClass( 'blink' );
	REFRESHDATA();
} );
$( '.i-search' ).on( 'click', function() {
	var type = $( this ).parents( '#divwlan' ).length ? 'wlan' : 'bluetooth';
	$( '.helphead, #divinterface, #divwebui' ).addClass( 'hide' );
	$( '#divscan'+ type )
		.removeClass( 'hide' )
		.find( 'i' ).eq( 0 ).addClass( 'blink' );
	SCAN[ type ]();
} );
$( '.scan' ).on( 'click', 'li:not( .current )', function() {
	var $this    = $( this );
	if ( $this.parent()[ 0 ].id === 'scanbluetooth' ) {
		clearTimeout( V.timeoutscan );
		COMMON.loader();
		BT( 'pair', $this );
		return
	}
	
	var ssid     = $this.data( 'id' );
	var security = $this.data( 'wpa' ) === 'wep';
	var encrypt  = $this.data( 'encrypt' ) === 'on';
	if ( $this.hasClass( 'profile' ) ) {
		S.ap ? WLAN.ap2ssid( ssid ) : CHANGE.ssid( ssid );
		return
	}
	
	INFO( {
		  icon    : 'wifi'
		, title   : ssid
		, message : encrypt ? false : V.i_warning +'Unsecured access point'
		, list    : encrypt ? [ 'Password', 'password' ] : false
		, oklabel : 'Connect'
		, ok      : () => {
			clearTimeout( V.timeoutscan );
			var val = { ESSID: ssid }
			if ( encrypt ) val = { ...val, IP: 'dhcp', KEY: _INFO.val(), SECURITY: security }
			S.ap ? WLAN.ap2ssid( ssid, val ) : WLAN.connect( val );
			$( '.divscan heading .blink' ).removeClass( 'blink' );
			$this.find( 'i' ).eq( 0 ).addClass( 'blink' );
		}
	} );
} );
$( '.entries:not( .scan )' ).on( 'click', 'li', function( e ) {
	var $li = $( this );
	if ( MENU.isActive( $li, e ) ) return
	
	V.bluetooth = V.lan = V.wlan = false;
	V[ $li.parent()[ 0 ].id ] = true;
	$( '#menu a' ).addClass( 'hide' );
	if ( V.bluetooth ) {
		var connected = $li.find( 'grn' ).length === 1;
		$( '#menu .connect' ).toggleClass( 'hide', connected );
		$MENU.find( '.forget, .info' ).removeClass( 'hide' );
		$MENU.find( '.disconnect, .rename' ).toggleClass( 'hide', ! connected );
	} else if ( V.lan ) {
		$( '#menu .edit' ).removeClass( 'hide' );
	} else if ( $li.hasClass( 'ap' ) ) {
		$( '#menu .info' ).removeClass( 'hide' );
	} else {
		var connected = $li.data( 'ip' ) !== undefined;
		$( '#menu a' ).removeClass( 'hide' );
		$( '#menu .connect' ).toggleClass( 'hide', connected );
		$( '#menu .disconnect' ).toggleClass( 'hide', ! connected );
		$( '#menu .rename' ).addClass( 'hide' );
	}
	MENU.show( $li );
} );
$( '.i-add' ).on( 'click', function() {
	EDIT[ $( this ).parents( '#divlan' ).length ? 'lan' : 'wlan' ]();
} );
$( '#menu a' ).on( 'click', function( e ) {
	var cmd = MENU.command( $( this ), e );
	if ( ! cmd ) return

	var $li = $( 'li.active' );
	if ( V.bluetooth ) {
		BT( cmd, $li );
		return
	}
	
	switch ( cmd ) {
		case 'connect':
			var ssid = $li.data( 'id' );
			if ( ! S.ap || V.localhost ) {
				CHANGE.ssid( ssid );
				return
			}
			
			if ( S.ap ) {
				if ( V.localhost ) {
					INFO( {
						  icon    : 'ap'
						, title   : 'Access Point'
						, message : 'Disconnect all clients?'
						, ok      : () => CHANGE.ssid( ssid )
					} );
				} else {
					WLAN.ap2ssid( ssid );
				}
			} else {
				CHANGE.ssid( ssid );
			}
			break
		case 'disconnect':
			if ( $li.data( 'ip' ) !== location.hostname ) {
				WLAN.disconnect();
				return
			}
			
			INFO( {
				  icon       : 'wifi'
				, title      : 'Wi-Fi'
				, message    : 'SSID: <wh>'+ ssid +'</wh>'
				, footer     : WLAN.warning( 'Disconnect' )
				, okcolor    : V.orange
				, ok         : WLAN.disconnect
			} );
			break
		case 'edit':
			if ( V.wlan ) {
				V.edit = $li.data( 'ip' ) ? true : 'editonly';
				SETTING( 'wlanprofile "'+ $li.data( 'id' ) +'"', values => EDIT.wlan( values ) );
			} else {
				EDIT.lan();
			}
			break
		case 'forget':
			var ssid = $li.data( 'id' );
			var icon = 'wifi';
			INFO( {
				  icon       : icon
				, title      : 'Wi-Fi'
				, message    : 'SSID: <wh>'+ ssid +'</wh>'
				, footer     : WLAN.warning( 'Forget' )
				, oklabel    : ICON( 'remove' ) +'Forget'
				, okcolor    : V.red
				, ok         : () => {
					NOTIFY( icon, ssid, 'Forget ...' );
					BASH( [ 'profileforget', ssid, 'CMD SSID' ] );
					$li.find( 'i' ).addClass( 'blink' );
				}
			} );
			break
		case 'info':
			STATUS( $li.parent()[ 0 ].id, $li.data( 'id' ), 'info' );
			break
		case 'rename':
			var icon = 'bluetooth';
			var name = $li.data( 'name' );
			INFO( {
				  icon         : icon
				, title        : 'Rename'
				, message      : '<wh>'+ name +'</wh>'
				, list         : [ 'As', 'text' ]
				, checkchanged : true
				, checkblank   : true
				, values       : name
				, ok           : () => {
					NOTIFY( icon, name, 'Change ...' );
					BASH( [ 'btrename', name, _INFO.val(), 'CMD NAME NEWNAME' ] );
					$li.find( 'i' ).addClass( 'blink' );
				}
			} );
			break
	}
} );

} );
